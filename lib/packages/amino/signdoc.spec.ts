...
describe("encoding", () => {
  describe("sortedJsonStringify", () => {
    it("leaves non-objects unchanged", () => {
      ...
      expect(sortedJsonStringify([5, ["a", "b"], true, null, 1])).toEqual(
        `[5,["a","b"],true,null,1]`,
      );
    });
    ...
    it("sorts objects in arrays", () => {
      ...
      // not yet sorted
      expect(
        sortedJsonStringify([1, 2, { b: { z: true, x: true, y: true }, a: true, c: true }, 4]),
      ).toEqual(`[1,2,{"a":true,"b":{"x":true,"y":true,"z":true},"c":true},4]`);
    });
  });
