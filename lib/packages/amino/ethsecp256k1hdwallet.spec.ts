...
describe("EthSecp256k1HdWallet", () => {
  ...
  describe("fromMnemonic", () => {
    it("works", async () => {
      const wallet = await EthSecp256k1HdWallet.fromMnemonic(defaultMnemonic);
  ...
  describe("generate", () => {
    it("defaults to 12 words", async () => {
      const wallet = await EthSecp256k1HdWallet.generate();
      expect(wallet.mnemonic.split(" ").length).toEqual(12);
    });

    it("can use different mnemonic lengths", async () => {
      expect((await EthSecp256k1HdWallet.generate(12)).mnemonic.split(" ").length).toEqual(12);
      expect((await EthSecp256k1HdWallet.generate(15)).mnemonic.split(" ").length).toEqual(15);
      expect((await EthSecp256k1HdWallet.generate(18)).mnemonic.split(" ").length).toEqual(18);
      expect((await EthSecp256k1HdWallet.generate(21)).mnemonic.split(" ").length).toEqual(21);
      expect((await EthSecp256k1HdWallet.generate(24)).mnemonic.split(" ").length).toEqual(24);
    });
  });

  describe("deserialize", () => {
    it("can restore", async () => {
      const original = await EthSecp256k1HdWallet.fromMnemonic(defaultMnemonic);
      const password = "123";
      const serialized = await original.serialize(password);
      const deserialized = await EthSecp256k1HdWallet.deserialize(serialized, password);
  ...
  describe("signAmino", () => {
    it("resolves to valid signature", async () => {
      ...
      const valid = await Secp256k1.verifySignature(
        ...
        keccak256(serializeSignDoc(signed)),
        defaultPubkey,
      );
