...
// const pubkeyAminoPrefixEthSecp256k1 = fromHex("615377f9" + "21" /* fixed length */);
const pubkeyAminoPrefixEthSecp256k1 = fromHex("f3b3cd03" + "21" /* fixed length */);
...
export function decodeAminoPubkey(data: Uint8Array): Pubkey {
  if (arrayContentStartsWith(data, pubkeyAminoPrefixSecp256k1)) {
    ...
    throw new Error(
      "Unsupported public key type. Amino data starts with: " + toHex(data.slice(0, 5)),
    );
  }
}
