...
import { fromBase64, toBase64 } from "@/lib/packages/encoding";
...
export function encodeEthSecp256k1Signature(
  ...
  if (signature.length !== 65) {
    throw new Error(
      "Signature must be 65 bytes long. Cosmos SDK uses a 2x32 byte fixed length encoding for the secp256k1 signature integers r, s, and recovery.",
    );
  }
