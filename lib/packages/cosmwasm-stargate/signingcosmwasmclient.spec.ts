...
import { Secp256k1HdWallet } from "@/lib/packages/amino";
import { sha256 } from "@/lib/packages/crypto";
import { toHex, toUtf8 } from "@/lib/packages/encoding";
import { decodeTxRaw, DirectSecp256k1HdWallet, Registry } from "@/lib/packages/proto-signing";
import {
  AminoMsgDelegate,
  AminoTypes,
...
import { AuthInfo, TxBody, TxRaw } from "@/lib/packages/cosmjs-types/cosmos/tx/v1beta1/tx";
import { MsgExecuteContract, MsgStoreCode } from "@/lib/packages/cosmjs-types/cosmwasm/wasm/v1/tx";
...
describe("SigningCosmWasmClient", () => {
  ...
  describe("updateAdmin", () => {
    it("can update an admin", async () => {
      ...
      const wasmClient = await makeWasmClient(wasmd.endpoint);
      const { contractInfo: contractInfo1 } =
        await wasmClient.wasm.getContractInfo(contractAddress);
  ...
  describe("migrate", () => {
    it("works", async () => {
      ...
      const options = { ...defaultSigningClientOptions, prefix: wasmd.prefix };
      const client = await SigningCosmWasmClient.connectWithSigner(wasmd.endpoint, wallet, options);
      const { codeId: codeId1 } = await client.upload(
        alice.address0,
        getHackatom().data,
        defaultUploadFee,
      );
      const { codeId: codeId2 } = await client.upload(
        alice.address0,
        getHackatom().data,
        defaultUploadFee,
      );
      ...
      it("works with a custom registry and custom message", async () => {
        ...
        const CustomMsgDelegate = {
          ...
          fromPartial(object: DeepPartial<CustomMsgDelegate>): CustomMsgDelegate {
            ...
            if (
              object.customDelegatorAddress !== undefined &&
              object.customDelegatorAddress !== null
            ) {
            ...
            if (
              object.customValidatorAddress !== undefined &&
              object.customValidatorAddress !== null
            ) {
