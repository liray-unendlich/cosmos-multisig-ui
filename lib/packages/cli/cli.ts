...
export async function main(originalArgs: readonly string[]): Promise<void> {
  ...
  const visiblePackages = (await installedPackages()).filter(
    (name) => name.startsWith("@/lib/packages/") || name === "axios",
  );
  ...
  if (args.selftest) {
    ...
    init += `
      ...
      import { sha512, Bip39, Random } from "@/lib/packages/crypto";
      import {
        coins,
        encodeAminoPubkey,
        encodeBech32Pubkey,
      ...
      import { assert, arrayContentEquals, sleep } from "@/lib/packages/utils";
