...
import { fromHex, toAscii } from "@/lib/packages/encoding";
...
describe("Libsodium", () => {
  describe("Argon2id", () => {
    ...
    it("works for 1 MiB memory and opsLimit = 5", async () => {
      ...
      await Argon2id.execute("123", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("3c5d010180ba0cf5b6b858cba23b318e42d33088983c404598599c3b029ecac6"),
        ),
      );
      await Argon2id.execute("!'§$%&/()", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("b0268bd63015c3d8f866f9be385507b466a9bfc75f271c2c1e97c00bf53224ba"),
        ),
      );
      await Argon2id.execute("ö", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("b113fc7863dbc87b7d1366c3b468d3864a2473ce46e90ed3641fff87ada561f7"),
        ),
      );
      await Argon2id.execute("😎", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("dc92db2a69a5607a75472e1581ac0851292ed9a2606f1000f62fa2efc97964e0"),
        ),
      );
    });
    ...
    it("works for 8 MiB memory and opsLimit = 2", async () => {
      ...
      await Argon2id.execute("123", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("3ee950488d26ce691657b1d753f562139857b61a58f234d6cb0ce84c4cc27328"),
        ),
      );
      await Argon2id.execute("!'§$%&/()", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("ab410498b44942a28f9d0dde72f0398edf104021ee41bb80412464975817a8a1"),
        ),
      );
      await Argon2id.execute("ö", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("f80c502bc3fe7b191f6e7e06359955d5dbd23f532548b7058ecbcf77a58e683d"),
        ),
      );
      await Argon2id.execute("😎", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("474d9445596d2600ba3dc9bbe87d21ed4879e2445cafb10fcb69c5c3ab8ecbc7"),
        ),
      );
    });
    ...
    it("works for 10 MiB memory and opsLimit = 1", async () => {
      ...
      await Argon2id.execute("123", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("f1832edbd41c209546eafd01f3aae28390de39bc13ff38981c4fc0c1ceaa05e3"),
        ),
      );
      await Argon2id.execute("!'§$%&/()", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("30c74f405d148fd5c882a0f4238aad9ed85ef255adc102411d22736d68f76f76"),
        ),
      );
      await Argon2id.execute("ö", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("b80a62f11e7a058194a8ddd80d341c47e0f3b6c41c72ee15b7926788e9963e8f"),
        ),
      );
      await Argon2id.execute("😎", salt, options).then((result) =>
        expect(result).toEqual(
          fromHex("b868aa1875de2edc57bc22de1fc75f9d19f451067c529565f73c61958088b5e9"),
        ),
      );
    });
  ...
  describe("Ed25519", () => {
    ...
    it("verifies signatures", async () => {
      ...
      {
        // wrong pubkey
        const otherSeed = fromHex(
          "91099374790843e29552c3cfa5e9286d6c77e00a2c109aaf3d0a307081314a09",
        );
  ...
  describe("Xchacha20poly1305Ietf", () => {
    ...
    it("decryption fails with wrong ciphertext/key/nonce", async () => {
      ...
      {
        // baseline
        expect(await Xchacha20poly1305Ietf.decrypt(ciphertext, key, nonce)).toEqual(
          originalMessage,
        );
      }
      {
