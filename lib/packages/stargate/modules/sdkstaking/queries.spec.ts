...
import { coin, coins, DirectSecp256k1HdWallet } from "@/lib/packages/proto-signing";
import { Tendermint34Client } from "@/lib/packages/tendermint-rpc";
import { sleep } from "@/lib/packages/utils";
...
import { SdkStakingExtension, setupSdkStakingExtension } from "./queries";

async function makeClientWithStaking(
  rpcUrl: string,
): Promise<[QueryClient & SdkStakingExtension, Tendermint34Client]> {
  const tmClient = await Tendermint34Client.connect(rpcUrl);
  return [QueryClient.withExtensions(tmClient, setupSdkStakingExtension), tmClient];
}

describe("SdkStakingExtension", () => {
  const defaultFee = {
  ...
  describe("validatorUnbondingDelegations", () => {
    it("works", async () => {
      ...
      const response = await client.staking.validatorUnbondingDelegations(
        validator.validatorAddress,
      );
      expect(response.unbondingResponses).toBeDefined();
      expect(response.unbondingResponses).not.toBeNull();
