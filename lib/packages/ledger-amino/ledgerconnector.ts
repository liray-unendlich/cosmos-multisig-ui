...
import { HdPath, Secp256k1Signature } from "@/lib/packages/crypto";
...
export class LedgerConnector {
  ...
  public async getHexAddress(pubkey?: Uint8Array): Promise<string> {
    const pubkeyToUse = pubkey || (await this.getPubkey());
    return pubkeyToAddressHex(encodeSecp256k1Pubkey(pubkeyToUse));
  }
